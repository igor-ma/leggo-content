...
def separa_em_blocos(pares_palavra_tag): 
	...
	i = 0
	while(i < len(pares_palavra_tag)):
		#documento.append(pares_palavra_tag[i])
		if("B-" in pares_palavra_tag[i][1]):
			documento.append(pares_palavra_tag[i])
			i += 1
			while(i < len(pares_palavra_tag) and "I-" in pares_palavra_tag[i][1]):
				documento.append(pares_palavra_tag[i])
				i += 1
			documentos.append(documento)
			documento = []
		else: #se for 'O'
			while(i < len(pares_palavra_tag) and "O" == pares_palavra_tag[i][1]):
				documento.append(pares_palavra_tag[i])
				i += 1
			documentos.append(documento)
			documento = []
...
def resultados_parciais(arquivo_modelo, X_teste, y_teste): #PESSOA, LEGISLACAO, TEMPO, ORGANIZACAO, JURISPRUDENCIA, LOCAL, e é sempre apenas B-<tag> e I-<tag>
	'''
	...
	labels_possiveis = ['O', 'B-PESSOA', 'I-PESSOA', 'B-LEGISLACAO', 'I-LEGISLACAO', 'B-TEMPO', 'I-TEMPO', 'B-ORGANIZACAO', 'I-ORGANIZACAO', 'B-JURISPRUDENCIA', 'I-JURISPRUDENCIA', 'B-LOCAL', 'I-LOCAL']
...
def main():
	...
	pares_palavra_tag = leitura_de_dados('NER_dados.txt')
